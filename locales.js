(function (root, factory) {
  ...
  const keys = {
    en: {
      ...
      'health.picker.selectDate': 'Choose a date',
      'health.picker.selectTime': 'Choose a time',
      'health.picker.chooseDate': 'Choose date',
      'health.picker.chooseBedtime': 'Choose bedtime',
      'health.picker.chooseWakeTime': 'Choose wake time',
      'health.picker.chooseQuality': 'Choose sleep quality',
      'health.picker.qualityOptions': 'Sleep quality options',
      'health.picker.previousMonth': 'Previous month',
      'health.picker.nextMonth': 'Next month',
      'health.picker.today': 'Today',
      'health.picker.hour': 'Hour',
      'health.picker.minute': 'Minute',
      'health.picker.cancel': 'Cancel',
      ...
      'health.connection.setupBody': 'Install the companion app, sign in with this account, grant Mi Fitness access, then tap Sync now on your phone.',
      'health.connection.refreshHint': 'Refresh loads data already uploaded by the Android companion.',
      'health.connection.confirmDisconnect': 'Disconnect this source? Existing health history will be retained.',
      'health.connection.disconnectedNotice': 'Connection disabled. Reconnect from the Android companion to sync again.',
      'health.esp32.endpoint': 'ESP32 data URL',
      'health.esp32.endpointPlaceholder': 'http://esp32.local/data',
      'health.esp32.setupTitle': 'Set ESP32 data URL',
      'health.esp32.saveAndSync': 'Save and sync',
      'health.esp32.sync': 'Sync ESP32 data',
      'health.esp32.syncing': 'Syncing…',
      'health.esp32.syncedShort': 'ESP32 synced',
      'health.esp32.fetching': 'Reading the ESP32 database…',
      'health.esp32.synced': 'Synced {{count}} readings; skipped {{skipped}} invalid rows.',
      'health.esp32.invalidUrl': 'Enter a valid HTTP or HTTPS ESP32 data URL without credentials.',
      'health.esp32.mixedContent': 'An HTTPS page cannot read an HTTP ESP32 endpoint. Use HTTPS or open this app over HTTP on the LAN.',
      'health.esp32.loginRequired': 'Log in before syncing ESP32 data.',
      'health.esp32.connectionRequired': 'Connect an active wearable source before syncing ESP32 data.',
      'health.esp32.responseTooLarge': 'The ESP32 response is larger than 5 MB.',
      'health.esp32.tooManySamples': 'The ESP32 response contains more than 50,000 samples.',
      'health.esp32.noValidSamples': 'No valid samples were found. Ensure utc_ms contains Unix epoch milliseconds.',
      'health.esp32.fetchFailed': 'Could not read the ESP32. Check its URL, network connection, and CORS headers.',
      'health.esp32.timeout': 'The ESP32 did not respond within 10 seconds.',
      'health.esp32.httpError': 'The ESP32 returned HTTP {{status}}.',
      'health.environment.temperature': 'Temperature',
      'health.environment.humidity': 'Humidity',
      'health.environment.light': 'Light intensity',
      'health.environment.noise': 'Noise',
      'health.analysis.title': 'Health Analysis',
      'health.analysis.rangeLabel': 'Analysis period',
      'health.analysis.export': 'Export CSV',
      'health.export.success': 'Health analysis exported.',
      ...
      'health.chart.environmentClimate': 'Temperature and humidity',
      'health.chart.environmentExposure': 'Light and noise',
      'health.sensor.title': 'Recent sensor session',
      'health.sensor.raw': 'Raw reading',
      'health.sensor.trend': '5-minute cubic trend',
      'health.sensor.meta': '{{start}} – {{end}} · {{count}} samples · median cadence {{cadence}}',
      'health.sensor.empty': 'No synchronized sensor session is available yet.',
      'health.sensor.demo': 'Demo data: these synthetic signals are for presentation only.',
      'health.sensor.warmup': 'The raw signal is available. The cubic trend starts after five complete minutes of sufficiently dense readings.',
      'health.sensor.truncated': 'Only the newest 10,000 samples in this session are shown.',
      'health.sensor.disclaimer': 'The cubic trend is a smoothing aid, not a prediction, diagnosis, or confidence interval. Raw readings remain visible.',
    ...
    'zh-CN': {
      ...
      'health.picker.selectDate': '选择日期',
      'health.picker.selectTime': '选择时间',
      'health.picker.chooseDate': '选择日期',
      'health.picker.chooseBedtime': '选择入睡时间',
      'health.picker.chooseWakeTime': '选择起床时间',
      'health.picker.chooseQuality': '选择睡眠质量',
      'health.picker.qualityOptions': '睡眠质量选项',
      'health.picker.previousMonth': '上个月',
      'health.picker.nextMonth': '下个月',
      'health.picker.today': '今天',
      'health.picker.hour': '小时',
      'health.picker.minute': '分钟',
      'health.picker.cancel': '取消',
      ...
      'health.connection.setupBody': '安装配套应用，使用本账号登录，授权读取 Mi Fitness 数据，然后在手机上点击“立即同步”。',
      'health.connection.refreshHint': '刷新只会加载安卓配套应用已上传的数据。',
      'health.connection.confirmDisconnect': '要断开此数据源吗？已有健康历史将被保留。',
      'health.connection.disconnectedNotice': '连接已停用。请在安卓配套应用中重新连接后再同步。',
      'health.esp32.endpoint': 'ESP32 数据地址',
      'health.esp32.endpointPlaceholder': 'http://esp32.local/data',
      'health.esp32.setupTitle': '设置 ESP32 数据地址',
      'health.esp32.saveAndSync': '保存并同步',
      'health.esp32.sync': '同步 ESP32 数据',
      'health.esp32.syncing': '正在同步…',
      'health.esp32.syncedShort': 'ESP32 已同步',
      'health.esp32.fetching': '正在读取 ESP32 数据库…',
      'health.esp32.synced': '已同步 {{count}} 条数据，跳过 {{skipped}} 条无效记录。',
      'health.esp32.invalidUrl': '请输入不含账号密码的有效 HTTP 或 HTTPS ESP32 数据地址。',
      'health.esp32.mixedContent': 'HTTPS 页面无法读取 HTTP ESP32 地址。请使用 HTTPS，或通过局域网 HTTP 打开本应用。',
      'health.esp32.loginRequired': '请先登录再同步 ESP32 数据。',
      'health.esp32.connectionRequired': '请先连接有效的可穿戴数据源再同步 ESP32 数据。',
      'health.esp32.responseTooLarge': 'ESP32 返回内容超过 5 MB。',
      'health.esp32.tooManySamples': 'ESP32 返回的数据超过 50,000 条。',
      'health.esp32.noValidSamples': '未找到有效样本，请确保 utc_ms 为 Unix 毫秒时间戳。',
      'health.esp32.fetchFailed': '无法读取 ESP32，请检查地址、网络连接和 CORS 响应头。',
      'health.esp32.timeout': 'ESP32 在 10 秒内没有响应。',
      'health.esp32.httpError': 'ESP32 返回 HTTP {{status}}。',
      'health.environment.temperature': '温度',
      'health.environment.humidity': '湿度',
      'health.environment.light': '光照强度',
      'health.environment.noise': '噪声',
      'health.analysis.title': '健康分析',
      'health.analysis.rangeLabel': '分析周期',
      'health.analysis.export': '导出 CSV',
      'health.export.success': '健康分析已导出。',
      ...
      'health.chart.environmentClimate': '温度与湿度',
      'health.chart.environmentExposure': '光照与噪声',
      'health.sensor.title': '最近传感器会话',
      'health.sensor.raw': '原始读数',
      'health.sensor.trend': '5 分钟三次多项式趋势',
      'health.sensor.meta': '{{start}} 至 {{end}} · {{count}} 个样本 · 中位采样间隔 {{cadence}}',
      'health.sensor.empty': '暂无已同步的传感器会话。',
      'health.sensor.demo': '演示数据：这些合成信号仅用于展示。',
      'health.sensor.warmup': '原始信号已显示。获得完整且密度足够的 5 分钟数据后才会显示三次多项式趋势。',
      'health.sensor.truncated': '当前会话仅显示最新的 10,000 个样本。',
      'health.sensor.disclaimer': '三次多项式趋势仅用于平滑显示，不代表预测、诊断或置信区间；原始读数始终保留。',
