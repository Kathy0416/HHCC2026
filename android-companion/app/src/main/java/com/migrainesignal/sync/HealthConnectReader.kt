...
class HealthConnectReader(private val context: Context) {
    ...
    suspend fun discoverOrigins(days: Long = 30): List<String> {
        ...
        if (HealthPermission.getReadPermission(SleepSessionRecord::class) in granted) readAll(SleepSessionRecord::class, filter).forEach { origins += it.metadata.dataOrigin.packageName }
        if (HealthPermission.getReadPermission(HeartRateRecord::class) in granted) readAll(HeartRateRecord::class, filter).forEach { origins += it.metadata.dataOrigin.packageName }
        if (HealthPermission.getReadPermission(OxygenSaturationRecord::class) in granted) readAll(OxygenSaturationRecord::class, filter).forEach { origins += it.metadata.dataOrigin.packageName }
        if (HealthPermission.getReadPermission(StepsRecord::class) in granted) readAll(StepsRecord::class, filter).forEach { origins += it.metadata.dataOrigin.packageName }
        return origins.filter(String::isNotBlank).sortedWith(compareByDescending<String> { isLikelyMiFitness(it) }.thenBy { it })
    }

    suspend fun readBundle(sourcePackage: String, days: Long = 90, zoneId: ZoneId = ZoneId.systemDefault()): SyncBundle {
        require(sourcePackage.isNotBlank()) { "Choose a Health Connect data source first" }
        val granted = grantedPermissions()
        val filter = TimeRangeFilter.between(Instant.now().minus(Duration.ofDays(days)), Instant.now())
        val origin = setOf(DataOrigin(sourcePackage))
        val sleeps = if (HealthPermission.getReadPermission(SleepSessionRecord::class) in granted) {
            readAll(SleepSessionRecord::class, filter, origin).map { record ->
                SleepInput(
                    ...
                    stages = record.stages.groupBy { stageName(it.stage) }.mapValues { (_, stages) -> stages.sumOf { Duration.between(it.startTime, it.endTime).toMinutes().toInt() } },
                    dataOrigin = record.metadata.dataOrigin.packageName
                )
            }
        } else emptyList()
        val heartRates = if (HealthPermission.getReadPermission(HeartRateRecord::class) in granted) {
