...
// Server upload (Wi-Fi → backend). serverUrl and deviceToken are configured
// through the setup portal and persisted in Preferences.
constexpr uint32_t UPLOAD_CHECK_INTERVAL_MS = 30000;  // 已联网且已配置时，每 30s 扫描待传事件
constexpr uint16_t UPLOAD_TIMEOUT_MS = 15000;         // 单次 HTTP 超时
constexpr size_t MAX_UPLOAD_BYTES = 128UL * 1024UL;   // 单个事件最大体积（完整事件约 58KB）
