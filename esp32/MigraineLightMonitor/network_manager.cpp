...
void WifiProvisioningManager::begin(uint64_t nowUs) {
  ...
  if (!preferencesReady_) {
    ...
    serverUrl_ = preferences_.getString("srv", "");
    deviceToken_ = preferences_.getString("devtoken", "");
  }
...
void WifiProvisioningManager::sendSetupPage(const char *message) {
  ...
  page += F("<title>Migraine Monitor Wi-Fi</title><style>body{font-family:sans-serif;max-width:30rem;margin:3rem auto;padding:0 1rem;color:#172033}input,button{box-sizing:border-box;width:100%;padding:.8rem;margin:.35rem 0 1rem;font-size:1rem}button{background:#3758d6;color:white;border:0;border-radius:.4rem}.note{background:#eef2ff;padding:.8rem;border-radius:.4rem}.id{background:#f4f6fb;padding:.8rem;border-radius:.4rem;font-family:monospace}</style></head><body>");
  page += F("<h1>Migraine monitor</h1><p>Connect to a 2.4 GHz Wi-Fi network, and optionally configure server upload.</p>");
  if (message != nullptr) {
    page += F("<p class=note>");
    page += message;
    page += F("</p>");
  }
  page += F("<p class=id>Device ID: ");
  page += deviceId;
  page += F("</p>");
  page += F("<form method=post action=/save>");
  page += F("<label>Network name (SSID)</label><input name=ssid maxlength=32 autocomplete=off>");
  page += F("<label>Wi-Fi password</label><input name=password type=password maxlength=63 autocomplete=off>");
  page += F("<label>Server URL (e.g. http://192.168.1.20:3000)</label><input name=serverUrl maxlength=128 autocomplete=off value=\"");
  page += serverUrl_;
  page += F("\">");
  page += F("<label>Device token</label><input name=deviceToken maxlength=128 autocomplete=off value=\"");
  page += deviceToken_;
  page += F("\">");
  page += F("<button type=submit>Save</button></form>");
  page += F("<p>The setup network closes after a successful connection. Monitoring continues during setup.</p></body></html>");
...
void WifiProvisioningManager::handlePortalSave(uint64_t nowUs) {
  ...
  const String serverUrl = webServer_.arg("serverUrl");
  const String deviceToken = webServer_.arg("deviceToken");

  // 服务器配置是可选字段，允许只更新服务器地址/令牌而不改动 Wi-Fi。
  if (serverUrl.length() > 0 || deviceToken.length() > 0) {
    if (preferencesReady_) {
      preferences_.putString("srv", serverUrl);
      preferences_.putString("devtoken", deviceToken);
    }
    serverUrl_ = serverUrl;
    deviceToken_ = deviceToken;
    Serial.println("[INFO] Server upload config saved");
  }

  if (ssid.length() == 0) {
    sendSetupPage(
        "Server configuration saved. Enter a network name above to connect Wi-Fi.");
    return;
  }
...
String WifiProvisioningManager::uploadUrl() const {
  String base = serverUrl_;
  while (base.endsWith("/")) {
    base.remove(base.length() - 1);
  }
  return base + "/api/hardware/events";
}
...
void WifiProvisioningManager::printStatus(Print &output) const {
  ...
  output.print("server_configured=");
  output.println(hasServerConfig() ? "true" : "false");
  if (serverUrl_.length() > 0) {
    output.print("server_url=");
    output.println(serverUrl_);
  }
}
