...
class DailyAggregatorTest {
    @Test
    fun `sleep uses wake date and same-day signals are summarized`() {
        val bundle = DailyAggregator.aggregate(
            ...
            steps = listOf(StepInterval(Instant.parse("2026-08-20T16:00:00Z"), 3000), StepInterval(Instant.parse("2026-08-20T17:00:00Z"), 2500))
        )
        ...
        assertEquals(5500L, day.steps)
        assertEquals("2026-08-21", bundle.sleepSessions.single().localDate.toString())
    }

    @Test
    fun `missing signal families remain nullable`() {
        val bundle = DailyAggregator.aggregate(
    ...
    @Test
    fun `mi fitness origins sort ahead of unrelated sources`() {
        assertEquals(true, HealthConnectReader.isLikelyMiFitness("com.mi.health"))
        assertEquals(true, HealthConnectReader.isLikelyMiFitness("com.xiaomi.wearable"))
        assertEquals(false, HealthConnectReader.isLikelyMiFitness("com.example.tracker"))
    }
}
