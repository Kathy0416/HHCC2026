(function () {
  ...
  function renderLocalAnalysis() {
    ...
    let calendar = {};
    try { calendar = JSON.parse(localStorage.getItem('migraineCalendarData') || '{}') || {}; } catch (error) { calendar = {}; }
    const series = Array.from({ length: state.range }, (_, index) => {
      const date = localDate(index - state.range + 1); const record = records.get(date); const cal = calendar[date] || {};
      return { date, migraine: !!cal.migraine, hasDiaryEntry: !!(cal.diary && String(cal.diary).trim()), stressTrigger: Array.isArray(cal.triggers) && cal.triggers.includes('stress'),
        sleepMinutes: record?.duration?.totalMinutes ?? null, sleepSource: record ? 'manual' : null,
        heartRateAvg: record?.heartRate ?? null, spo2Avg: record?.spo2 ?? null, steps: record?.steps ?? null };
    });
    const sleepValues = series.map((day) => day.sleepMinutes).filter(Number.isFinite);
    const hrValues = series.map((day) => day.heartRateAvg).filter(Number.isFinite);
    const spo2Values = series.map((day) => day.spo2Avg).filter(Number.isFinite);
    const stepValues = series.map((day) => day.steps).filter(Number.isFinite);
    const migraineDays = series.filter((day) => day.migraine).length;
    renderAnalysis({ range: state.range, series, kpis: { averageSleepMinutes: sleepValues.length ? sleepValues.reduce((a, b) => a + b, 0) / sleepValues.length : null, migraineDays, averageHeartRate: hrValues.length ? hrValues.reduce((a, b) => a + b, 0) / hrValues.length : null, averageSpo2: spo2Values.length ? spo2Values.reduce((a, b) => a + b, 0) / spo2Values.length : null, averageSteps: stepValues.length ? stepValues.reduce((a, b) => a + b, 0) / stepValues.length : null }, coverage: { recordedDays: sleepValues.length, overlappingDays: 0, insightsAvailable: false }, comparisons: null });
  }
